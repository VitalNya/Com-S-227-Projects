...
/**
 * UI for a maze search application. A client (in this case the SwingWorker
 * thread) will periodically call updateStatus to trigger a repaint of the maze.
 */
public class MazePanel extends JPanel implements MazeObserver {
	...
	/**
	 * Constructs a panel to display the given maze, using the given value
	 * <code>sleepTime</code> as a delay between frames.
	 * 
	 * @param maze      the grid to be displayed
	...
	/**
	 * Start up the maze search algorithm using an auxiliary thread.
	 */
	...
	/**
	 * Triggers a repaint and pauses the auxiliary execution thread for 'sleepTime'
	 * milliseconds.
	 */
	@Override
	...
	private Color getColor(int row, int col) {
		...
		if (m == null)
			return Color.WHITE;
		if (maze.getStartColumn() == col && maze.getStartRow() == row)
			return Color.BLUE;
		if (m.isWall())
			return Color.BLACK;
		if (m.isGoal())
	...
	private boolean isExploring(MazeCell c) {
		...
		return (s == CellStatus.SEARCHING_UP || s == CellStatus.SEARCHING_LEFT || s == CellStatus.SEARCHING_DOWN
				|| s == CellStatus.SEARCHING_RIGHT);
	}
