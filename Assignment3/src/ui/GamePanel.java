...
public class GamePanel extends JPanel
{
   ...
   */
  public GamePanel(AlphabetSoup game, ScorePanel scorePanel, boolean useAnimation, boolean verbose)
  {
    this.game = game;
  ...
  private void paintTile(Graphics g, Tile t)
  {
    ...
    String text = "" + (char) ('a' + t.getCurrentValue() - 1);
    int h = fm.getHeight();
  ...
  {
    int value = t.getCurrentValue() - 1;
    
    // clamp to range if necessary
    int index = Math.max(Math.min(value, GameMain.colors.length - 1), 0);
   ...
   * @return
   */
  private boolean createNormalizedMoves(ArrayList<ShiftDescriptor> moves, ArrayList<NormalizedMove> normalizedMoves, Tile[][] before)
  {   
    for (ShiftDescriptor mm : moves)           
    {
      log(mm.toString());
      ...
      if (m.getOldPosition2() != null)
      {
        ...
        int expectedValueOfDestinationCell = m.getNewValue() - 1;
  ...
  /**
   * Converts the ShiftDescriptor object returned by the game into a normalized form
   * with actual row and column indices.
   * @param mv
   * @return
   */
  private NormalizedMove makeMoveFromMiniMove(ShiftDescriptor mv)
  {
    ...
    // For AlphabetSoup game, oldPos2 would always be newPos
  ...
  private class MyKeyListener implements KeyListener
  {
    ...
    public void keyPressed(KeyEvent event)
    {
      ...
      else
      {
        ...
        ArrayList<ShiftDescriptor> moves = game.shift(dir);
        ...
        else
        {
          ...
          if (!validMoves)
          {
            log("ERROR: unable to attempt animation due to errors in descriptors. ");
            animationSetupOk = false;
          }
    ...
    public void keyReleased(KeyEvent event)
    {
      ...
      TileInfo newTilePosition = game.setNewTile();

      if (newTilePosition == null)
      {
        log("INFO: setNewTile() returned null TileInfo");
        tiles.clear();
        for (int row = 0; row < game.getSize(); row += 1)
        {
          for (int col = 0; col < game.getSize(); col += 1)
      ...
      {
        log("INFO: setNewTile() returned " + newTilePosition);
        int actual = game.getCell(newTilePosition.getRow(),  newTilePosition.getCol());
        if (newTilePosition.getValue() != actual)
        {
          log("WARNING: setNewTile return value appears to be inconsistent with grid.");
        }
      }
