...
public class Donuts
{
   ...
   */
  private static double donutHelper(int coffees, int donuts) //Coffee and donuts were mixed around. coffee values went to the donuts and donut val went to coffee
  {
    int dozens = donuts / 12;
    int singles = donuts % 12;
    int freeCoffees = dozens * 2;
    coffees = coffees - freeCoffees; // No such thing as a negative coffee. If statments makes coffees 0 if lower or equal to 0.
     if (coffees <= 0) {
    	 coffees = 0;
    }
