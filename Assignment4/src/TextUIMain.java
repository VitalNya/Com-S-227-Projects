...
public class TextUIMain {
	 ...
	 * 
	 * @param g game to use in this UI
	 */
	...
	/**
	 * Prints the score categories for the game; if argument is true, includes
	 * potential scores for the current roll of the dice.
	 * 
	 * @param dice dice group representing current state of the dice, possibly null
	 */
	private void printCategories(boolean printPotentialScores) {
		...
		if (game.getUpperSection().size() > 0) {
			...
			System.out.printf(totalFormat3, "Upper section total:",
					game.getUpperSectionTotal() + game.getUpperSectionBonus(), game.getUpperSectionTotal(),
	...
	/**
	 * Allows a user to select a category in which to score the current roll of the
	 * dice. (Categories are represented in the console window as numbers.)
	 * 
	...
	/**
	 * Allows a user to select which dice should be added to those completed and
	 * which should be rolled again.
	 */
	 ...
	 * 
	 * @return array of scoring categories
	 */
